...
class TestDb(object):
    ...
    def test_create(self, userexists, dbexists):
        ...
        pg.psql('-c', "SELECT 1 FROM pg_roles WHERE rolname='user';",
        dboverrides=dboverrides, envoverrides=envoverrides
                ).AndReturn('1\n' if userexists else '')
        if not userexists:
            pg.psql('-c', "CREATE USER user WITH PASSWORD 'pass';",
            dboverrides=dboverrides, envoverrides=envoverrides)

        pg.psql('-c', "SELECT 1 FROM pg_database WHERE datname='name';",
        dboverrides=dboverrides, envoverrides=envoverrides
                ).AndReturn('1\n' if dbexists else '')
        if not dbexists:
            pg.psql('-c', "CREATE DATABASE name WITH OWNER user;",
            dboverrides=dboverrides, envoverrides=envoverrides)
